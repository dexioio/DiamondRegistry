 ...
 * DetailedERC721 token implementation works as expected.
 */
...
contract('DetailedERC721Token', accounts => {
  ...
  describe('tokenOfOwnerByIndex', function () {
    describe('when the given address owns some tokens', function () {
      ...
      describe('given index lower than amount of tokens owned by the address', 
      function () {
        const index = 0;
      ...
      describe('index greater than or equal to total tokens owned by address',
      function () {
        const index = 2;
  ...
  describe('tokenMetadata', function () {
    ...
    describe('when some metadata was set', function () {
      it('returns the metadata of the given token', async function () {
        await token.setTokenMetadata(_firstTokenId, 'dummy metadata', 
          { from: _creator });
  ...
  describe('setTokenMetadata', function () {
    describe('when the sender is not the token owner', function () {
      ...
      it('reverts', async function () {
        await assertRevert(token.setTokenMetadata(_firstTokenId, 'new metadata', 
          { from: sender }));
      });
    });
    ...
    describe('when the sender is the owner of the token', function () {
      ...
      describe('when the given token ID was tracked by this contract before', 
      function () {
        const tokenId = _firstTokenId;

        it('updates the metadata of the given token ID', async function () {
          await token.setTokenMetadata(_firstTokenId, 'new metadata', 
            { from: sender });
        ...
        it('emits a metadata updated event', async function () {
          const { logs } = await token.setTokenMetadata(tokenId, 'new metadata',
           { from: sender });
      ...
      describe('when given token ID was not tracked by this contract before', 
      function () {
        const tokenId = _unknownTokenId;

        it('reverts', async function () {
          await assertRevert(token.setTokenMetadata(tokenId, 'new metadata'), 
            { from: sender });
        });
      });
    });
